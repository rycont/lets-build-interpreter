...
export function tokenizer(_code) {
	...
	// tokens에서 Token 다음에 Operator { value: ":" }가 오는 곳을 찾아야 합니다
	for (let i = 0; i < tokens.length - 1; i++) {
		// Token 다음에 Operator가 오는 곳을 찾았습니다
		if (
			tokens[i] instanceof Token && // Token이고
			tokens[i + 1] instanceof Keyword && // 그 다음에 Operator가 오고
			tokens[i + 1].value === ':' // Operator의 value가 ":"라면
		) {
